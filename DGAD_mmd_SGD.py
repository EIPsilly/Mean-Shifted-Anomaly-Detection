...
def test(model, train_loader, test_loader):
    train_feature_space = []
    with torch.no_grad():
        for (idx, imgs, augimg, label, domain_labels) in tqdm(train_loader, desc='Train set feature extracting'):
            imgs = imgs.cuda()
            features = model(imgs)
    ...
    for key in test_loader:
        ...
        test_labels = []
        with torch.no_grad():
            for (idx, imgs, augimg, labels, domain_labels) in tqdm(test_loader[key], desc='Test set feature extracting'):
                imgs = imgs.cuda()
                features = model(imgs)
...
def get_score(model, device, train_loader, test_loader):
    ...
    test_labels = []
    with torch.no_grad():
        for (idx, imgs, augimg, labels, domain_label) in tqdm(test_loader, desc='Test set feature extracting'):
            imgs = imgs.to(device)
            features = model(imgs)
            test_feature_space.append(features)
            test_labels.append(labels)
...
if __name__ == "__main__":
    ...
    # args = parser.parse_args(["--lambda0", "1", "--epochs", "10", "--lr", "0.001"])
    ...
    if args.dataset == "PACS":
        filename = f'dataset={args.dataset},normal_class={args.normal_class},anomaly_class={args.anomaly_class},epochs={args.epochs},lr={args.lr},batch_size={args.batch_size},backbone={args.backbone},contamination_rate={args.contamination_rate},lambda0={args.lambda0},cnt={args.cnt}'
    if args.dataset == "MVTEC":
