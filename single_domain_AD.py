...
def calc_score(score_net, cluster_centers, dataloader, domain_key):
    for sample in train_loader:
        ...
        image = image[torch.where(target == 0)[0]]
        normal_image = image[0]
        break
    
    normal_image = normal_image.cuda().unsqueeze(0)
    model.eval()
    score_net.eval()
    test_loss = 0.0
    total_pred = np.array([])
    ...
    for i, sample in enumerate(dataloader):
        ...
        with torch.no_grad():
            invariant_feature = model.inference(img1, normal_image)
...
def main(args):
    ...
    print(device)
    global model
    model = net_work.Align_Test_Model(args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_type", type=str, default="EFDM")
