...
class Align_Test_Model(torch.nn.Module):
    ...
    def inference(self, x, normal):
        normal = normal.expand(x.shape)
        ...
        if self.args.test_type == "EFDM":
            normal = self.backbone.conv1(normal)
            normal = self.backbone.bn1(normal)
            normal = self.backbone.relu(normal)
            normal = self.backbone.maxpool(normal)
            normal = self.backbone.layer1(normal)
            x1 = EFDM_test(x1, normal)

        x2 = self.backbone.layer2(x1)
        if self.args.test_type == "EFDM":
            normal = self.backbone.layer2(normal)
            x2 = EFDM_test(x2, normal)
