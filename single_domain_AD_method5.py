...
def test(model, train_loader, test_loader):
    ...
    for key in test_loader:
        ...
        distances = net_work2.knn_score(train_feature_space, test_feature_space)
...
def get_score(model, device, train_loader, test_loader):
    ...
    with torch.no_grad():
        for (idx, imgs, augimg, label, _) in tqdm(train_loader, desc='Train set feature extracting'):
            ...
            features1, features2 = model.normal_feature_sample(imgs)
            features1_list.append(features1)
            features2_list.append(features2)
        train_feature_space = torch.cat(train_feature_space, dim=0).contiguous().cpu().numpy()
        features1_list = torch.cat(features1_list)
        features2_list = torch.cat(features2_list)
    test_feature_space = []
    test_labels = []
    with torch.no_grad():
        for (idx, imgs, augimg, labels, _) in tqdm(test_loader, desc='Test set feature extracting'):
            imgs = imgs.to(device)
            features = model.inference(imgs, features1_list, features2_list)
            test_feature_space.append(features)
            test_labels.append(labels)
        test_feature_space = torch.cat(test_feature_space, dim=0).contiguous().cpu().numpy()
        test_labels = torch.cat(test_labels, dim=0).cpu().numpy()
